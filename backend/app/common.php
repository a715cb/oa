...
function export_csv($fileName = '', $headArr = [], $data = [])
{
    ...
    // SEC-014: 对表头进行 CSV 公式注入防护
    $headArr = array_map('escape_csv_cell', $headArr);
    fputcsv($fp, $headArr);
    $index = 0;
    foreach ($data as $item) {
        ...
        $index++;
        // SEC-014: 对数据单元格进行 CSV 公式注入防护
        $item = array_map('escape_csv_cell', $item);
        fputcsv($fp, $item);
    }
    ob_flush();
    flush();
...
/**
 * SEC-014: CSV 公式注入防护函数
 * 防止以 =、+、-、@ 开头的内容在 Excel 中被当作公式执行
...
/**
 * 获取客户ip地址
 * SEC-021 修复：使用 ThinkPHP 提供的 request()->ip() 方法
 * 避免使用可被客户端伪造的 HTTP_X_FORWARDED_FOR 和 HTTP_CLIENT_IP
 *
 * @return string
 */
function get_client_ip($type = 0)
{
    ...
    // SEC-021: 优先使用 ThinkPHP 提供的 IP 获取方法
    // 该方法会自动处理代理 IP 的可信来源验证
    $ip = request()->ip();

    // 如果 ThinkPHP 方法未获取到，则使用 REMOTE_ADDR（最可靠）
