...
class Permissions {
        ...
        if (is_string($name)) {
            ...
            if (strpos($name, ',') !== false) {
                ...
                if ($findAuthRuleCount == 0) {
                    // SEC-009 修复：未注册的权限节点默认拒绝访问（Fail-Safe Default）
                    return false;
                }
                $name = [$name];
            }
     ...
     */
    private function checkSuperAdmin(int $uid): bool
    {
        $superAdminRoleId = config('system.super_admin_id');
        if (empty($superAdminRoleId)) {
            return false;
        }
        
        // 获取用户角色
        $userRoles = Db::name($this->user_role_db)
            ->where('user_id', $uid)
            ->column('role_id');
        
        return in_array($superAdminRoleId, $userRoles);
    }
