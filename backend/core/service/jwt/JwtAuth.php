...
class JwtAuth
{
    ...
    /**
     * 获取请求中的Token
     * 仅允许从 Authorization 请求头获取，防止 Token 通过 URL 参数泄露
     * 
     * @return string
     * @throws TokenInvalidException 当 Authorization 头为空时抛出
     */
    public function getToken(): string
    {
        ...
        if (empty($authHeader)) {
            throw new TokenInvalidException('缺少认证令牌');
        }

        $token = str_replace('Bearer ', '', $authHeader);
        
        if (empty($token)) {
            throw new TokenInvalidException('无效的认证令牌');
        }
